...
async function fetchInformationLayer(
  geometry: any, 
  departement: string,
  config: FetchConfig
) {
  ...
  if (center) {
    ...
    try {
      ...
      console.error(`❌ [Information] Erreur National ${config.nationalLayer}`, err);
    }
  }
...
export const getInformationsSurf = async (geometry: any, departement: string) => {
  return fetchInformationLayer(geometry, departement, {
    nationalLayer: 'wfs_du:info_surf',
    localSuffix: 'info_surf'
  });
};

export const getInformationsLin = async (geometry: any, departement: string) => {
  return fetchInformationLayer(geometry, departement, {
    nationalLayer: 'wfs_du:info_lin',
    localSuffix: 'info_lin'
  });
};

export const getInformationsPct = async (geometry: any, departement: string) => {
  return fetchInformationLayer(geometry, departement, {
    nationalLayer: 'wfs_du:info_pct',
    localSuffix: 'info_pct'
  });
};