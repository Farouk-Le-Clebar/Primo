...
export const filterByFilters = (
    ...
    if (filters.parametersMin) {
        filtered = filtered.filter(
            (p) => p.parameters >= parseInt(filters.parametersMin),
        );
    }
    if (filters.parametersMax) {
        filtered = filtered.filter(
            (p) => p.parameters <= parseInt(filters.parametersMax),
        );
    }

    if (filters.parcelsMin) {
        filtered = filtered.filter(
            (p) => p.parcels >= parseInt(filters.parcelsMin),
        );
    }
    if (filters.parcelsMax) {
        filtered = filtered.filter(
            (p) => p.parcels <= parseInt(filters.parcelsMax),
        );
    }
    ...
    if (filters.timeRange && filters.timeRange !== "custom") {
        ...
        filtered = filtered.filter((p) => {
            ...
            return (
                modDate >= new Date(filters.customStartDate) &&
