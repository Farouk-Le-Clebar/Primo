...
export default function ParcelSearchModal({ isOpen, onClose, navigationData, onNavigate }: SearchModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-start justify-center pt-[15vh] px-4 animate-in fade-in duration-150" onClick={onClose}>
      ...
      <div className="relative w-full max-w-[600px] bg-white dark:bg-[#111111] rounded-xl shadow-2xl border border-gray-200 dark:border-[#232323] overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center px-4 py-3.5 border-b border-gray-100 dark:border-[#232323]">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-[14px] outline-none text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 font-inter"
          />
          
          <div className="flex items-center gap-1.5 border border-gray-200 dark:border-[#333333] rounded-md px-2 py-1 bg-gray-50/50 dark:bg-[#232323] select-none ml-2">
            <span className="text-[10px] font-mono font-bold text-gray-400 dark:text-gray-500">ESC</span>
          </div>
        </div>

        <div className="max-h-[400px] overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-[#333333] scrollbar-track-transparent">
          {query === "" ? (
            ...
            filtered.length > 0 ? filtered.map((item) => (
              <button
                ...
                className="flex w-full items-center justify-between px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-[#1C1C1C] text-gray-700 dark:text-gray-300 transition-colors"
              >
                <div className="flex items-center gap-3">
                  <span className="text-[14px] font-mono text-gray-300 dark:text-gray-600 w-4 text-center">#</span>
                  <span className="text-[13px] font-medium">{item.label}</span>
                </div>
                <span className="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wide font-medium">{item.category}</span>
              </button>
            )) : (
              <div className="px-4 py-8 text-center text-gray-400 dark:text-gray-500 text-[12px] italic">Aucun module ne correspond à "{query}"</div>
            )
          )}
        </div>
      </div>
