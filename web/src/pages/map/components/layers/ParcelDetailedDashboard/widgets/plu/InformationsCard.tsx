    ...
    <Card className="p-0 border-gray-200 dark:border-[#232323] ring-0 shadow-sm overflow-y-auto font-inter">
      <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-[#232323] bg-white dark:bg-[#171717]">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white">Informations annexes</h3>
        </div>
        {!isPending && !isEmpty && (
          <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">{uniqueInformations.length} éléments</span>
        )}
      </div>

      {isPending ? (
        <div className="flex items-center justify-center p-8 bg-white dark:bg-[#171717]">
          <LoadingPrimoLogo className="w-6 h-6 text-black dark:text-white" />
        </div>
      ) : isEmpty ? (
        <div className="p-8 text-sm text-[#878D96] text-center bg-gray-50 dark:bg-[#171717] dark:text-gray-400">
                  ...
                  <TableCell className="py-4 px-0">
                    <div className="flex flex-col gap-0.5">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[13px] font-semibold text-gray-900 dark:text-white leading-none">{props.libelle}</span>
                      </div>
                      <div className="max-h-0 opacity-0 group-hover:max-h-10 group-hover:opacity-100 transition-all duration-300 overflow-hidden">
                        <p className="text-[11px] text-gray-500 dark:text-gray-400 italic pl-6 pt-1">
                          {humanDescription}
                        </p>
                      </div>
                    </div>
