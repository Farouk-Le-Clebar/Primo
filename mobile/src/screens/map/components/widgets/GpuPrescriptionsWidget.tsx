...
import { getPrescriptionDescription, getPrescriptionColorBar } from '../../../../utils/gpu/prescriptions';

interface GpuPrescriptionsWidgetProps {
  feature: any;
}

const KIND_LABELS: Record<string, string> = { surface: 'Surface', lineaire: 'Linéaire', ponctuel: 'Ponctuel' };

export default function GpuPrescriptionsWidget({ feature }: GpuPrescriptionsWidgetProps) {
  ...
  return (
    <WidgetCard
      ...
      isEmpty={!isPending && uniquePrescriptions.length === 0}
    ...
    >
      {uniquePrescriptions.map((presc: any, index: number) => {
        ...
        const barColor = getPrescriptionColorBar(props.typepsc, props.libelle);
        const description = getPrescriptionDescription(props.typepsc, props.libelle);
        return (
            ...
            <View style={{ paddingLeft: 8 }}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', paddingTop: 6, borderTopWidth: 1, borderTopColor: '#f8fafc' }}>
                <Text style={{ fontSize: 10, color: '#94a3b8', fontWeight: '500', textTransform: 'capitalize' }}>
                  {KIND_LABELS[presc._kind] || presc._kind}
                </Text>
